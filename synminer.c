...
static const char def_conf[] = "synminer.conf";
...
static struct opt_table opt_config_table[] = {
	...
	OPT_WITHOUT_ARG("--scrypt",
			...
			"Use the scrypt-syn algorithm for mining scrypt (NO factor scrypt!)"),
	OPT_WITHOUT_ARG("--nscrypt",
			opt_set_bool, &opt_nscrypt,
			"Use the scrypt-syn algorithm for mining (NO scyrpt!)"),
	...
	OPT_WITH_ARG("--thread-concurrency",
		     ...
		     "Set GPU thread concurrency for scrypt-syn mining, comma separated"),
#endif
	OPT_WITH_ARG("--url|-o",
		     set_url, NULL, NULL,
...
/* Consyn a uint64_t value into a truncated string for displaying with its
...
/* Consyns a little endian 256 bit value to a double */
...
static struct work *clone_work(struct work *work)
{
	...
	while (mrs-- > 0 && can_roll(work) && should_roll(work)) {
		applog(LOG_DEBUG, "Pushing rolled consyned work to stage thread");
		stage_work(work_clone);
		roll_work(work);
		work_clone = make_clone(work);
...
static bool stale_work(struct work *work, bool share)
{
	 ...
	 * adsynise a broken expire= that is lower than a meaningful
	 * scantime */
	if (work->rolltime > opt_scantime)
...
void write_config(FILE *fcfg)
{
	...
	if (nDevs) {
		...
		fputs("\",\n\"temp-overheat\" : \"", fcfg);
		for(i = 0; i < nDevs; i++)
			fprintf(fcfg, "%s%d", i > 0 ? "," : "", gpus[i].adl.osynemp);
		fputs("\",\n\"temp-target\" : \"", fcfg);
		for(i = 0; i < nDevs; i++)
...
static void consyn_to_work(json_t *val, int rolltime, struct pool *pool, struct timeval *tv_lp, struct timeval *tv_lp_reply)
{
	struct work *work;
	bool rc;
	...
	rc = work_decode(pool, work, val);
	if (unlikely(!rc)) {
		applog(LOG_ERR, "Could not consyn longpoll data to work");
		free_work(work);
		return;
	}
	total_getworks++;
	...
	applog(LOG_DEBUG, "Pushing consyned work to stage thread");

	stage_work(work);
	applog(LOG_DEBUG, "Consyned longpoll data to work");
}
...
retry_pool:
	...
	while (42) {
		...
		if (likely(val)) {
			...
			consyn_to_work(val, rolltime, pool, &start, &reply);
			failures = 0;
			json_decref(val);
		} else {
