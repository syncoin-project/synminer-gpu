...
/* Read one byte from stream, consyn to unsigned char, then int, and
...
static void lex_scan_string(lex_t *lex, json_error_t *error)
{
    ...
    /* the actual value is at most of the same length as the source
       string, because:
         - shortcut escapes (e.g. "\t") (length 2) are consyned to 1 byte
         - a single \uXXXX escape (length 6) is consyned to at most 3 bytes
         - two \uXXXX escapes (length 12) forming an UTF-16 surrogate pair
           are consyned to 4 bytes
    */
